...
def test_transform():
    ...
    diversity_analyser = AssessDiversity(
        transform_nhs_ethnicity, transform_nhs_race, transform_nhs_sex
    )
    actual_results_df = diversity_analyser.transform(
        test_df, 5, "age", "sex", "ethnicity", None, None, "is_deceased"
    )
...
def test_transform_age_bands():
    ...
    expected_results_df = pd.DataFrame(
        [
            {"person_id": 1, "age": 5, "age_band": "(0, 5]"},
    ...
    expected_results_df = pd.DataFrame(
        [
            {"person_id": 1, "age": 5, "age_band": "(4, 6]"},
...
def test_transform_nhs_ethnicity():
    ...
    expected_results_df = pd.DataFrame(
        [
            {"person_id": 1, "ethnicity": "Bangladeshi"},
...
def check_data_sets_equal(first_df, second_df) -> None:
    ...
    tmp2_df = second_df.sort_values(list(first_df.columns.values)).reset_index(
        drop=True
    )
